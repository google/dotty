...
"""
Legacy parser inherited from the EFILTER prototype in Rekall.

Slashy is getting replaced by dotty (in experiments). At present, Rekall still
relies on the slash ('/') operator yielding slightly different AST from the
arrow ('->') for the code that makes decisions about which entity collectors to
run. (In dotty both operators are unified in the dot ('.')).

For everyone who isn't Rekall, the recommended parser is already dotty. Slashy
is going to go the way of the dodo as soon as Rekall stops requiring it (soon).
"""
...
INFIX = collections.OrderedDict([
    ("->", Operator(precedence=2, assoc="left", handler=ast.Let,
...
syntax.Syntax.register_parser(Parser, shorthand="slashy")
